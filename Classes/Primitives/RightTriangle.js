...
export default class RightTriangle extends Primitive {
    ...
    drawMeasures(ctx, offsetX, offsetY, n, size) {
        ...
        this.measureLine(ctx, points[0].x, points[0].y, points[2].x, points[0].y, 0, -10, this.width)
        this.measureLine(ctx, points[0].x, points[0].y, points[1].x, points[1].y, 20, 0, this.height)
        this.measureLine(ctx, points[0].x, points[0].y, points[2].x, points[2].y, -20, -20 * raio, t.toFixed(1))
        ctx.beginPath();
        ctx.strokeStyle = '#000';
        let fp = points[0];
