export default class BaseDrawer {
  ...
  removeAnyPointsOnShape(shape) {
    this.rows.forEach((row) => {
      for (let x of row.points) {
        if (
          (x >= shape.x && x < shape.x + shape.width && shape.y == row.height) ||
  ...
  isInTheShape(primitive, x, y) {
    ...
    for (let i = 0; i < sz; i++) {
      if (this.primitives.some((s) => s.isInside({ x: primitive.points[i].x + x, y: primitive.points[i].y + y }))) return true;
    }
    return false;
  }
  ...
  addShape(primitive) {
    ...
    primitive.points.forEach((p, index) => {
      if (index && p.x >= 0 && p.y >= 0 && p.x < this.width && !this.isOnTheShape(p.x, p.y)) this.addPointInRows(p);
    });
